...
0,5000,5104,5219,5346,5485,5637,5802,5981,6174,6382,6610,6860,7134,7434,7762,8120,8510,8934,9394,9892,10430,11010,11634,12304,13022,13790,14610,15484,16414,17402,18450,19560,20734,21974,23282,24660,26110,27634,29234,30912,32670,34510,36434,38444,40542,42730,45010,47384,49854,52422,55090,57860,60734,63714,66802,70000,73310,76734,80274,83932,87710,91610,95634,99784,104062,108470,113010,117684,122494,127442,132530,137760,143134,148654,154322,160140,166110,172234,178514,184952,191550,198310,205234,212324,219582,227010,234610,242384,250334,258462,266770,275260,283934,292794,301842,311080,320510,330134,339954];
    ...
    function readPriceMemory(){
      try{
        const raw=lsGet(PRICE_MEMORY_KEY);
        const p=raw?JSON.parse(raw):null;
        return (p&&typeof p==='object')?p:{tiers:{},lastSigKey:''};
      }catch(e){return {tiers:{},lastSigKey:''}}
    }
    function writePriceMemory(m){
      try{lsSet(PRICE_MEMORY_KEY,JSON.stringify(m))}catch(e){}
    }
    function median(arr){
    ...
    function analyzePriceMemory(){
      const rows=pricedRows();
      if(!rows.length)return null;
      const mem=readPriceMemory();if(!mem.tiers)mem.tiers={};
      const nowMs=now();let sig=null;
      for(const row of rows){
        const tm=mem.tiers[row.key]||{samples:[]};
        const prev=(tm.samples||[]).filter(s=>nowMs-(s.at||0)<7*24*3600000);
        const base=median(prev.map(s=>Number(s.usdtPer1k)));
      ...
      if(sig){
        mem.lastSigKey=sig.key;
        if(radarOn&&lastSigKey!==sig.key){
          lastSigKey=sig.key;
          const msg=sig.label+' is '+sig.drop.toFixed(1)+'% below usual ($'+sig.per1k.toFixed(2)+' /1k)';
          playPing();
          if(typeof Notification!=='undefined'&&Notification.permission==='granted'){
            try{new Notification('Temporal Deal Radar',{body:msg,silent:false});}catch(e){}
          }
        }
      }
      writePriceMemory(mem);
      return sig;
    }
    ...
    async function toggleRadar(){
      ...
      btn.setAttribute('aria-pressed',radarOn?'true':'false');
      if(radarOn&&typeof Notification!=='undefined'&&Notification.permission==='default'){
        try{await Notification.requestPermission()}catch(e){}
      }
      if(!radarOn){lastRadarKey='';lastSigKey=''}
      renderRadar();
    }
    ...
    function rRelic(){
      ...
      for(let i=0;i<4;i++){
        ...
        let h='<div class="relic-result-cp" style="color:'+hex+'">+'+fmtNum(b.totalCP)+' CP</div><div class="relic-result-pieces" style="color:'+hex+'">'+fmtNum(per[i])+'</div><div class="relic-result-price">'+(cost?relicFmt(cost):'\u2014')+'</div>';
        el.innerHTML=h;
      }
    ...
    function renderMkt(total){
      ...
      const clear=(txt)=>{state.textContent=txt;state.classList.add('show');chestSum.innerHTML='';tier.innerHTML='';};
      ...
      tier.innerHTML=ordered.map(row=>{
        ...
        return '<div class="tier-row'+(isBest?' best-value':'')+(isSel?' selected':'')+'" data-tier="'+row.key+'"><span class="tier-name"><span>'+row.key+'</span>'+(isBest?'<span class="value-tag">'+t('bestTag')+'</span>':'')+'</span><span class="tier-per1k">'+relicFmt(row.per1k)+t('per1k')+'</span><span class="tier-qty">'+fmtNum(need)+' '+t('chests')+'</span><span class="tier-cost">'+relicFmt(row.usdt*need)+'</span></div>';
      }).join('');
